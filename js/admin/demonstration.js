...
function currentTime(){
	...
	$el.find(".details span").last().html(dayNames[currentDate.getDay()] + ", " + currentDate.getHours()+":"+ ("0" + currentDate.getMinutes()).slice(-2) + ":" + currentDate.getSeconds() );
	setTimeout(function(){
		currentTime();
	}, 1);
}
...
if($("#simg").length > 0){
	function formatFlags(state){
		...
		return "<img style='padding-right:10px;' src='../images/admin/demo/flags/" + state.id.toLowerCase() + ".gif'/>" + state.text;
	}
	$("#simg").select2({
