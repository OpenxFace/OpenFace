...
$db_server   = $_SESSION['dbHost'];
$db_name     = $_SESSION['dbName'];
$db_username = $_SESSION['dbUsername'];
...
$filename           = INSTALL_SQL;     // Specify the dump filename to suppress the file selection dialog
...
$upload_dir = dirname(__FILE__.'/tmp');
...
<meta name="robots" content="noindex, nofollow">
	...
	<script type="text/javascript">
		...
		var DEFAULT_PRELOADER_IMAGE = '<?php echo DEFAULT_PRELOADER_IMAGE; ?>';
	</script>
...
<?php do_action('head_style'); ?>
    ...
    <script src="<?php echo BASEURL; ?>/js/jquery.blockUI.js" type="text/javascript"></script>
    <script src="<?php echo BASEURL; ?>/js/jquery.blockUI.defaults.js" type="text/javascript"></script>	
	<script src="<?php echo BASEURL; ?>/js/bootstrap/bootstrap.min.js" type="text/javascript"></script>
	<script src="<?php echo BASEURL; ?>/js/bootbox.min.js" type="text/javascript"></script>
</head>
...
// Start processing queries from $file
  ...
  { $query="";
    ...
    $linenumber = ( isset( $_REQUEST["start"] ) ) ? $_REQUEST["start"] : 1;
    $linenumber = ( (int)$linenumber == 0 ) ? 1 : $linenumber;
    $querylines=0;
    $inparents=false;
...
// Finish message and restart the script
    ...
    if ($linenumber<$_REQUEST["start"]+$linespersession) {      
      ...
      $error = true; // This is a semi-error telling the script that it is finished
      ...
      file_put_contents( BASEDIR.'/install.lock', date('m-d-Y') );
      
      ?>
      	<script type="text/javascript">
      		$(document).ready(function() {
      			$('#bigdumpBody').html('<div class="alert alert-success"><i class="fa fa-check-circle"></i> Installation complete!<br>Please <strong>delete the install directory</strong> located @ '+BASEDIR+' for security reasons.</div>');
      			$.unblockUI();		
      		});	
      	</script>
      <?php            
...
?>
	...
	$.blockUI();
	<?php $linenumber = ( (int)$linenumber == 0 ) ? 1 : $linenumber; ?>
