...
$('body').on('click', '.change-input', function(e) {
    ...
    $inputToClone.attr('readonly', false);    
    $parentCloned = $el.parent().clone();
    $parentCloned.html( $inputToClone.clone().val('') );
    // change clone ID
    $parentCloned.children().attr('id', $inputToClone.clone().attr('id') + 'Confirm' );
    // change clone name
    $parentCloned.children().attr('name', $inputToClone.clone().attr('name') + 'Confirm' );
...
$('body').on('click', '.update-input', function(e) {
    ...
    var inputFields = $parent.parent().find('input');
    var values		= new Array();
    $.each( inputFields, function( index, value ) {
    	var value = $(value).val();
    	values.push( value );
    });
    if( values[0] == values[1] && strlen( trim( values[0] ) ) ) {        
    	$parent.parent().parent().removeClass('error');
    $el.after('<span><i class="icon-spinner icon-spin"></i>Updating...</span>');
    setTimeout(function(){
        	var field = $parent.parent().find('input[type=text],textarea,select').filter(':visible:first');
        	field.val( values[0] );
        	field.attr('value', values[0] );        	
...
$(".theme-colors > li > span").hover(function(e){
 ...
 $.cookie('adminTemplateColor', $el.attr('class'), { expires: 3650, path: '/' });
});
