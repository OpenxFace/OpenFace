...
$.fn.themeswitcher = function(settings) {
	...
	$.fn.spShow = function(){ $(this).css({top: button.offset().top + options.buttonHeight, left: button.offset().left}).slideDown(50); button.css(button_active); options.onOpen(); }
	$.fn.spHide = function(){ $(this).slideUp(50, function(){options.onClose();}); button.css(button_default); }
	
		
	/* Theme Loading
	---------------------------------------------------------------------*/
	//switcherpane.find('a').live('click', function() {
	...
	var button_default = {
		...
		fontSize: '11px',
		color: '#666',
		background: '#eee url(img/themeswitcher/buttonbg.png) 50% 50% repeat-x',
	...
	.find('.jquery-ui-themeswitcher-icon').css({
		...
		width: '16px',
		height: '16px',
		background: 'url(img/themeswitcher/icon_color_arrow.gif) 50% 50% no-repeat'
	});	
	//pane css
	switcherpane.css({
		position: 'absolute',
	...
	.find('li').hover(
		function(){ 
			$(this).css({
				'borderColor':'#555',
				'background': 'url(img/themeswitcher/menuhoverbg.png) 50% 50% repeat-x',
				cursor: 'pointer'
			}); 
		},
		function(){ 
...
jQuery.cookie = function(name, value, options) {
    if (typeof value != 'undefined') { // name and value given, set cookie
        ...
        if( options.onSet ) {
        	options.onSet(name, value);
        }        
        // CAUTION: Needed to parenthesize options.path and options.domain
        // in the following expressions, otherwise they evaluate to undefined
        // in the packed version for some reason...
        var path	= options.cookiePath ? '; path=' + (options.cookiePath) : '';      
